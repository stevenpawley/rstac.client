...
validate_stac <- function(stac_object, strict = FALSE) {
  ...
  if (inherits(stac_object, "stac_item")) {
    result <- validate_item(stac_object, strict)
  } else if (inherits(stac_object, "stac_collection")) {
    result <- validate_collection(stac_object, strict)
  } else if (inherits(stac_object, "stac_catalog")) {
    result <- validate_catalog(stac_object, strict)
  } else {
...
validate_links <- function(links) {
  ...
  for (i in seq_along(links)) {
    ...
    if (!is.list(link)) {
      errors <- c(errors, paste0("Link[", i, "] must be an object"))
      next
    }
...
validate_assets <- function(assets) {
  ...
  for (key in asset_keys) {
    ...
    if (!is.list(asset)) {
      errors <- c(errors, paste0("Asset '", key, "' must be an object"))
      next
    }
    ...
    if (!is.null(asset$roles)) {
      if (!is.character(asset$roles) && !is.list(asset$roles)) {
        errors <- c(errors, paste0("Asset '", key, "' 'roles' must be an array"))
      }
    }
  }
...
validate_extent <- function(extent) {
  ...
  if (!is.list(extent)) {
    return("Field 'extent' must be an object")
  }
  ...
  if (is.null(extent$spatial)) {
    ...
    if (is.null(extent$spatial$bbox)) {
      ...
      errors <- c(errors, "Field 'extent.spatial.bbox' must be an array")
    } else {
  ...
  if (is.null(extent$temporal)) {
    ...
    } else if (!is.list(extent$temporal$interval)) {
      errors <- c(errors, "Field 'extent.temporal.interval' must be an array")
    } else {
      for (i in seq_along(extent$temporal$interval)) {
        interval <- extent$temporal$interval[[i]]
        if (length(interval) != 2) {
          errors <- c(errors, paste0(
            "extent.temporal.interval[", i, "] must have exactly 2 elements"
          ))
        }
      }
    }
...
validate_providers <- function(providers) {
  ...
  if (!is.list(providers)) {
    return("Field 'providers' must be an array")
  }
  ...
  for (i in seq_along(providers)) {
    ...
    if (!is.null(provider$roles)) {
      ...
      if (is.character(provider$roles)) {
        ...
        errors <- c(errors, paste0("Provider[", i, "] 'roles' must be an array of strings"))
      }
    }
  }
